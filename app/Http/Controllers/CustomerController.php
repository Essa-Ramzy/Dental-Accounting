        ...
        if (request()->ajax()) {
            if (request('search')) {
                if (request('filter') == 'all') {
                    $entries = Customer::where('id', 'like', '%' . request('search') . '%')
                        ->orWhere('name', 'like', '%' . request('search') . '%')->get();
                } else {
                    $entries = Customer::where(request('filter'), 'like', '%' . request('search') . '%')->get();
                }
            } else {
                $entries = Customer::all();
            }
            ...
            $footer = "
            <tr>
                <th scope=\"row\" colspan=\"5\" class=\"text-md-center\">Number of Customers: {$entries->count()}</th>
            </tr>";
