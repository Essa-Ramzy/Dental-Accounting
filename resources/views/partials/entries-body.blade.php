@forelse ($entries as $entry)
    <tr class="align-middle">
        ...
        @if ($trash)
            <td class="text-muted">
                {{ $entry->deleted_at->format('M d, Y H:i') }}</td>
        @else
            <td class="text-muted">
            ...
            <div class="btn-group" role="group" aria-label="Entry actions">
                ...
                @else
                    <a href="{{ route('Entry.edit', ['id' => $entry->id]) }}"
                        class="btn btn-sm btn-outline-primary border-secondary border-end-0 d-flex align-items-center d-flex align-items-center"
                ...
                <a href="#deleteModal" data-bs-toggle="modal" id="{{ $entry->id }}"
                    class="btn btn-sm btn-outline-danger border-secondary border-start-0 d-flex align-items-center d-flex align-items-center"
    ...
    <tr>
        <td colspan="11" class="text-center py-5 text-muted">
            ...
            @if ($trash)
                ...
                <a href="{{ route('Entry.index') }}" class="btn btn-primary d-inline-flex align-items-center gap-2">
            ...
            @else
                ...
                <a href="{{ route('Entry.create') }}" class="btn btn-primary d-inline-flex align-items-center gap-2">
                    <svg width="16" height="16" aria-hidden="true">
